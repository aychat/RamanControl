...
double nloptJ(unsigned N, const double *opt_params, double *grad_J, void *nloptJ_params)
{
    ...
    params->w_v = opt_params[2];
    params->w_R = opt_params[3];
    ...
    printf("%g %g %g %g %g\n", params->A_R, -params->time[0] / params->width_R, params->w_v * 27.211385, 27.211385 * params->w_R, J);
...
//------------------------------------------------------------//
{
    ...
    double x[4] =  {func_params->guess[0], func_params->guess[1], func_params->guess[2], func_params->guess[3]};
    double maxf;
