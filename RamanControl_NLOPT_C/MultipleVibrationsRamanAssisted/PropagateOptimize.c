...
//----------------------------------------------------//
{
    ...
    double A_R_1 = params->A_R_1;
    double A_R_2 = params->A_R_2;
    double width_R_1 = params->width_R_1;
    double width_R_2 = params->width_R_2;
    double t0_R_1 = params->t0_R_1;
    double t0_R_2 = params->t0_R_2;

    double A_EE = params->A_EE;
    double width_EE = params->width_EE;
    double t0_EE = params->t0_EE;

    double w_R_1 = params->w_R_1;
    double w_R_2 = params->w_R_2;
    double w_v_1 = params->w_v_1;
    double w_v_2 = params->w_v_2;
    double w_EE = params->w_EE;


    for(i=0; i<timeDIM; i++)
    {
        field[i] = A_R_1 * exp(-pow(t[i] - t0_R_1, 2) / (2. * pow(width_R_1, 2))) * (cos((w_R_1 + w_v_1) * t[i]) + cos(w_R_1 * t[i]))
        + A_R_2 * exp(-pow(t[i] - t0_R_2, 2) / (2. * pow(width_R_2, 2))) * (cos((w_R_2 + w_v_2) * t[i]) + cos(w_R_2 * t[i]));
...
{
    double molA_excited_pop = creal(molA->rho[1*nDIM + 1]);
    double molB_excited_pop = creal(molB->rho[1*nDIM + 1]);
...
double nloptJ(unsigned N, const double *opt_params, double *grad_J, void *nloptJ_params)
{
    ...
    params->A_R_1 = opt_params[0];
    params->A_R_2 = opt_params[1];
    params->w_v_1 = opt_params[2];
    params->w_v_2 = opt_params[3];
    params->w_R_1 = opt_params[4];
    params->w_R_2 = opt_params[5];
