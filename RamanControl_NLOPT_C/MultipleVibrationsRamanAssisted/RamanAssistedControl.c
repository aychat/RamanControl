...
//----------------------------------------------------//
{
    ...
    for(i=0; i<timeDIM; i++)
    {
        field[i] = A_R * exp(-pow(t[i] - t0_R, 2) / (2. * pow(width_R, 2))) * (cos((w_R + w_v1) * t[i]) + cos((w_R + w_v2) * t[i]) + cos(w_R * t[i]));
...
double calculateJ(molecule* molA, molecule* molB, int nDIM)
{
    ...
    double molB_excited_pop = creal(molB->rho[1*nDIM + 1]) + creal(molA->rho[2*nDIM + 2]);
...
double nloptJ(unsigned N, const double *opt_params, double *grad_J, void *nloptJ_params)
{
    ...
    printf("%g %g %g %g %g %g\n", params->A_R, -params->time[0] / params->width_R, params->w_v1 * 27.211385,  params->w_v2 * 27.211385, 27.211385 * params->w_R, J);
...
//------------------------------------------------------------//
{
    ...
    double x[5] =  {func_params->guess[0], func_params->guess[1], func_params->guess[2], func_params->guess[3], func_params->guess[4]};
    double maxf;
